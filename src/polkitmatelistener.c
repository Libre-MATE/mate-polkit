...
static void polkit_mate_listener_initiate_authentication(
...
static gboolean polkit_mate_listener_initiate_authentication_finish(
    PolkitAgentListener *listener, GAsyncResult *res, GError **error);

G_DEFINE_TYPE(PolkitMateListener, polkit_mate_listener,
              POLKIT_AGENT_TYPE_LISTENER);

static void polkit_mate_listener_init(PolkitMateListener *listener) {}

static void polkit_mate_listener_finalize(GObject *object) {
...
static void polkit_mate_listener_class_init(PolkitMateListenerClass *klass) {
...
static AuthData *auth_data_new(PolkitMateListener *listener,
                               PolkitMateAuthenticator *authenticator,
...
static void maybe_initiate_next_authenticator(PolkitMateListener *listener) {
  if (listener->active_authenticator == NULL &&
      listener->authenticators != NULL) {
    polkit_mate_authenticator_initiate(
        POLKIT_MATE_AUTHENTICATOR(listener->authenticators->data));
    listener->active_authenticator = listener->authenticators->data;
  }
}

static void authenticator_completed(PolkitMateAuthenticator *authenticator,
                                    gboolean gained_authorization,
                                    gboolean dismissed, gpointer user_data) {
  AuthData *data = user_data;

  data->listener->authenticators =
      g_list_remove(data->listener->authenticators, authenticator);
  ...
  if (dismissed) {
    g_task_return_new_error(
        data->task, POLKIT_ERROR, POLKIT_ERROR_CANCELLED,
...
static void polkit_mate_listener_initiate_authentication(
  ...
  task = g_task_new(G_OBJECT(listener), NULL, callback, user_data);
  g_task_set_source_tag(task, polkit_mate_listener_initiate_authentication);

  authenticator = polkit_mate_authenticator_new(action_id, message, icon_name,
  ...
  g_signal_connect(authenticator, "completed",
                   G_CALLBACK(authenticator_completed), data);
  ...
  listener->authenticators =
      g_list_append(listener->authenticators, authenticator);
