...
G_DEFINE_TYPE_WITH_PRIVATE(PolkitMateAuthenticationDialog,
                           polkit_mate_authentication_dialog, GTK_TYPE_DIALOG);
...
enum { PIXBUF_COL, TEXT_COL, USERNAME_COL, N_COL };

static void user_combobox_set_sensitive(GtkCellLayout *cell_layout,
...
static void user_combobox_changed(GtkComboBox *widget, gpointer user_data) {
  PolkitMateAuthenticationDialog *dialog =
      POLKIT_MATE_AUTHENTICATION_DIALOG(user_data);
  GtkTreeIter iter;
  gchar *user_name;

  if (gtk_combo_box_get_active_iter(GTK_COMBO_BOX(widget), &iter)) {
    gtk_tree_model_get(GTK_TREE_MODEL(dialog->priv->store), &iter, USERNAME_COL,
                       &user_name, -1);
...
static GdkPixbuf *get_user_icon(char *username) {
  GError *error;
  ...
  user_path = g_variant_get_string(
      g_variant_get_child_value(find_user_result, 0), NULL);

  get_icon_result = g_dbus_connection_call_sync(
...
static void create_user_combobox(PolkitMateAuthenticationDialog *dialog) {
   ...
   */
  if (dialog->priv->store != NULL) return;

  combo = GTK_COMBO_BOX(dialog->priv->user_combobox);
  dialog->priv->store =
      gtk_list_store_new(3, GDK_TYPE_PIXBUF, G_TYPE_STRING, G_TYPE_STRING);

  gtk_list_store_append(dialog->priv->store, &iter);
  gtk_list_store_set(dialog->priv->store, &iter, PIXBUF_COL, NULL, TEXT_COL,
                     _("Select user..."), USERNAME_COL, NULL, -1);
  ...
  for (i = 0, n = 0; dialog->priv->users[n] != NULL; n++) {
    ...
    if (passwd == NULL) {
      g_warning("Error doing getpwnam(\"%s\"): %s", dialog->priv->users[n],
                strerror(errno));
      continue;
    }
    ...
    if (gecos != NULL && strlen(gecos) > 0 &&
        strcmp(gecos, dialog->priv->users[n]) != 0)
    ...
    if (pixbuf == NULL) {
      pixbuf =
          gtk_icon_theme_load_icon(gtk_icon_theme_get_default(), "stock_person",
                                   GTK_ICON_SIZE_MENU, 0, NULL);
    }

    gtk_list_store_append(dialog->priv->store, &iter);
    gtk_list_store_set(dialog->priv->store, &iter, PIXBUF_COL, pixbuf, TEXT_COL,
                       real_name, USERNAME_COL, dialog->priv->users[n], -1);
  ...
  gtk_cell_layout_set_cell_data_func(GTK_CELL_LAYOUT(combo), renderer,
                                     user_combobox_set_sensitive, NULL, NULL);
...
static GtkWidget *get_image(PolkitMateAuthenticationDialog *dialog) {
  GdkPixbuf *pixbuf;
  ...
  vendor_pixbuf = gtk_icon_theme_load_icon(
      gtk_icon_theme_get_default(), dialog->priv->icon_name, 48, 0, NULL);
  if (vendor_pixbuf == NULL) {
    g_warning("No icon for themed icon with name '%s'",
  ...
  pixbuf = gtk_icon_theme_load_icon(gtk_icon_theme_get_default(),
                                    "dialog-password", 48, 0, NULL);
  if (pixbuf == NULL) goto out;

  /* need to copy the pixbuf since we're modifying it */
  copy_pixbuf = gdk_pixbuf_copy(pixbuf);
  if (copy_pixbuf == NULL) goto out;

  /* blend the vendor icon in the bottom right quarter */
  gdk_pixbuf_composite(vendor_pixbuf, copy_pixbuf, 24, 24, 24, 24, 24, 24, 0.5,
                       0.5, GDK_INTERP_BILINEAR, 255);
...
out:
  ...
  if (copy_pixbuf != NULL) g_object_unref(copy_pixbuf);

  if (vendor_pixbuf != NULL) g_object_unref(vendor_pixbuf);

  return image;
}

static void polkit_mate_authentication_dialog_set_property(GObject *object,
                                                           guint prop_id,
                                                           const GValue *value,
                                                           GParamSpec *pspec) {
  PolkitMateAuthenticationDialog *dialog =
      POLKIT_MATE_AUTHENTICATION_DIALOG(object);
...
static void polkit_mate_authentication_dialog_get_property(GObject *object,
                                                           guint prop_id,
                                                           GValue *value,
                                                           GParamSpec *pspec) {
  PolkitMateAuthenticationDialog *dialog =
...
static GtkWidget *add_row(GtkWidget *grid, int row, const char *label_text,
                          GtkWidget *entry) {
  GtkWidget *label;
...
static void polkit_mate_authentication_dialog_finalize(GObject *object) {
  PolkitMateAuthenticationDialog *dialog;
  ...
  if (dialog->priv->details != NULL) g_object_unref(dialog->priv->details);

  g_strfreev(dialog->priv->users);
  g_free(dialog->priv->selected_user);

  if (dialog->priv->store != NULL) g_object_unref(dialog->priv->store);

  if (G_OBJECT_CLASS(polkit_mate_authentication_dialog_parent_class)
          ->finalize != NULL)
    G_OBJECT_CLASS(polkit_mate_authentication_dialog_parent_class)
        ->finalize(object);
}

static GtkWidget *polkit_mate_dialog_add_button(GtkDialog *dialog,
...
static void polkit_mate_authentication_dialog_constructed(GObject *object) {
  PolkitMateAuthenticationDialog *dialog;
  GtkWidget *hbox;
  GtkWidget *main_vbox;
  GtkWidget *vbox;
  ...
  dialog->priv->cancel_button = polkit_mate_dialog_add_button(
      GTK_DIALOG(dialog), _("_Cancel"), "process-stop", GTK_RESPONSE_CANCEL);

  dialog->priv->auth_button = gtk_dialog_add_button(
      GTK_DIALOG(dialog), _("_Authenticate"), GTK_RESPONSE_OK);
  ...
  if (g_strv_length(dialog->priv->users) > 1) {
    gtk_label_set_markup(GTK_LABEL(label),
                         _("An application is attempting to perform an action "
                           "that requires privileges. "
                           "Authentication as one of the users below is "
                           "required to perform this action."));
  } else {
    if (strcmp(g_get_user_name(), dialog->priv->users[0]) == 0) {
      gtk_label_set_markup(
          GTK_LABEL(label),
          _("An application is attempting to perform an action that requires "
            "privileges. "
            "Authentication is required to perform this action."));
    } else {
      gtk_label_set_markup(GTK_LABEL(label),
                           _("An application is attempting to perform an "
                             "action that requires privileges. "
                             "Authentication as the super user is required to "
                             "perform this action."));
    }
  }
  ...
  if (g_strv_length(dialog->priv->users) > 1) {
    ...
    gtk_box_pack_start(GTK_BOX(main_vbox),
                       GTK_WIDGET(dialog->priv->user_combobox), FALSE, FALSE,
                       0);
  ...
  dialog->priv->prompt_label =
      add_row(grid_password, 0, _("_Password:"), dialog->priv->password_entry);
  ...
  /* initially never show the password entry stuff; we'll toggle it on/off so
   * it's only shown when prompting for a password */
  ...
  /* Details */
  details_expander =
      gtk_expander_new_with_mnemonic(_("<small><b>_Details</b></small>"));
  ...
  s = g_strdup_printf("<small><a href=\"%s\">%s</a></small>",
                      dialog->priv->action_id, dialog->priv->action_id);
  gtk_label_set_markup(GTK_LABEL(label), s);
  g_free(s);
  ...
  g_signal_connect(label, "activate-link", G_CALLBACK(action_id_activated),
                   NULL);
  ...
  gtk_label_set_use_markup(GTK_LABEL(label), TRUE);
  s = g_strdup_printf("<small><a href=\"%s\">%s</a></small>",
                      dialog->priv->vendor_url, dialog->priv->vendor);
  gtk_label_set_markup(GTK_LABEL(label), s);
  g_free(s);
  ...
  if (have_user_combobox && gtk_combo_box_get_active(GTK_COMBO_BOX(
                                dialog->priv->user_combobox)) == 0) {
...
static void polkit_mate_authentication_dialog_class_init(
    PolkitMateAuthenticationDialogClass *klass) {
  ...
  g_object_class_install_property(
      gobject_class, PROP_DETAILS,
      g_param_spec_object("details", NULL, NULL, POLKIT_TYPE_DETAILS,
                          G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY |
                              G_PARAM_STATIC_NAME | G_PARAM_STATIC_NICK |
                              G_PARAM_STATIC_BLURB));

  g_object_class_install_property(
      gobject_class, PROP_ACTION_ID,
      g_param_spec_string("action-id", NULL, NULL, NULL,
                          G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY |
                              G_PARAM_STATIC_NAME | G_PARAM_STATIC_NICK |
                              G_PARAM_STATIC_BLURB));

  g_object_class_install_property(
      gobject_class, PROP_VENDOR,
      g_param_spec_string("vendor", NULL, NULL, NULL,
                          G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY |
                              G_PARAM_STATIC_NAME | G_PARAM_STATIC_NICK |
                              G_PARAM_STATIC_BLURB));

  g_object_class_install_property(
      gobject_class, PROP_VENDOR_URL,
      g_param_spec_string("vendor-url", NULL, NULL, NULL,
                          G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY |
                              G_PARAM_STATIC_NAME | G_PARAM_STATIC_NICK |
                              G_PARAM_STATIC_BLURB));
  ...
  g_object_class_install_property(
      gobject_class, PROP_USERS,
      g_param_spec_boxed("users", NULL, NULL, G_TYPE_STRV,
                         G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY |
                             G_PARAM_STATIC_NAME | G_PARAM_STATIC_NICK |
                             G_PARAM_STATIC_BLURB));

  g_object_class_install_property(
      gobject_class, PROP_SELECTED_USER,
      g_param_spec_string("selected-user", NULL, NULL, NULL,
 ...
 **/
GtkWidget *polkit_mate_authentication_dialog_new(
    const gchar *action_id, const gchar *vendor, const gchar *vendor_url,
    const gchar *icon_name, const gchar *message_markup, PolkitDetails *details,
    gchar **users) {
  PolkitMateAuthenticationDialog *dialog;
  GtkWindow *window;

  dialog = g_object_new(POLKIT_MATE_TYPE_AUTHENTICATION_DIALOG, "action-id",
                        action_id, "vendor", vendor, "vendor-url", vendor_url,
                        "icon-name", icon_name, "message", message_markup,
                        "details", details, "users", users, NULL);
 ...
 *
 * Call this function to indicate an authentication error; typically shakes the
 *window
 **/
void polkit_mate_authentication_dialog_indicate_error(
    PolkitMateAuthenticationDialog *dialog) {
  int x, y;
  int n;
  int diff;
 ...
 *
 * Returns: %TRUE if a user is selected (use
 *polkit_mate_dialog_get_selected_user() to obtain the user) or %FALSE if the
 *dialog was cancelled.
 **/
gboolean polkit_mate_authentication_dialog_run_until_user_is_selected(
    PolkitMateAuthenticationDialog *dialog) {
  gboolean ret;
  gint response;
  ...
  if (response == RESPONSE_USER_SELECTED) ret = TRUE;
 ...
 *
 * Returns: The response (free with g_free()) or %NULL if one of @was_cancelled
 *or @new_user_selected has been set to %TRUE.
 **/
gchar *polkit_mate_authentication_dialog_run_until_response_for_prompt(
    PolkitMateAuthenticationDialog *dialog, const gchar *prompt,
    gboolean echo_chars, gboolean *was_cancelled, gboolean *new_user_selected) {
  gint response;
  gchar *ret;

  gtk_label_set_text_with_mnemonic(GTK_LABEL(dialog->priv->prompt_label),
                                   prompt);
  ...
  if (was_cancelled != NULL) *was_cancelled = FALSE;

  if (new_user_selected != NULL) *new_user_selected = FALSE;
  ...
  if (response == GTK_RESPONSE_OK) {
    ...
    if (new_user_selected != NULL) *new_user_selected = TRUE;
  } else {
    if (was_cancelled != NULL) *was_cancelled = TRUE;
  }
 ...
 *
 * Returns: The currently selected user (free with g_free()) or %NULL if no user
 *is currently selected.
 **/
gchar *polkit_mate_authentication_dialog_get_selected_user(
    PolkitMateAuthenticationDialog *dialog) {
  return g_strdup(dialog->priv->selected_user);
}

void polkit_mate_authentication_dialog_set_info_message(
    PolkitMateAuthenticationDialog *dialog, const gchar *info_markup) {
 ...
 **/
gboolean polkit_mate_authentication_dialog_cancel(
    PolkitMateAuthenticationDialog *dialog) {
  if (!dialog->priv->is_running) return FALSE;
