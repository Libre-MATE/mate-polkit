...
static void revoke_tmp_authz_cb(GObject *source_object, GAsyncResult *res,
                                gpointer user_data) {
  GError *error;

  error = NULL;
  polkit_authority_revoke_temporary_authorizations_finish(
      POLKIT_AUTHORITY(source_object), res, &error);
  if (error != NULL) {
...
static void revoke_tmp_authz(void) {
  polkit_authority_revoke_temporary_authorizations(authority, session, NULL,
                                                   revoke_tmp_authz_cb, NULL);
}
...
static void update_temporary_authorization_icon_real(void) {
   ...
   *   for subjects that no longer exists.. this is because temporary
   * authorizations are only valid for the subject that trigger the
   * authentication dialog.
   *
   *   Maybe the authority could do this, would probably involve some polling,
   * but it seems cleaner to do this server side.
   */
  ...
  if (current_temporary_authorizations != NULL) {
    ...
    if (app_indicator == NULL) {
      ...
      app_indicator = app_indicator_new("mate-polkit", "dialog-password",
    ...
    if (status_icon == NULL) {
      ...
      gtk_status_icon_set_tooltip_text(
          status_icon, _("Click the icon to drop all elevated privileges"));
      g_signal_connect(status_icon, "activate",
                       G_CALLBACK(on_status_icon_activate), NULL);
      g_signal_connect(status_icon, "popup-menu",
                       G_CALLBACK(on_status_icon_popup_menu), NULL);
    }
#endif
  } else {
    /* hide icon */
...
static void enumerate_temporary_authorizations_cb(GObject *source_object,
                                                  GAsyncResult *res,
                                                  gpointer user_data) {
  ...
  temporary_authorizations =
      polkit_authority_enumerate_temporary_authorizations_finish(authority, res,
                                                                 &error);
  if (error != NULL) {
...
static void update_temporary_authorization_icon(PolkitAuthority *authority) {
  polkit_authority_enumerate_temporary_authorizations(
      authority, session, NULL, enumerate_temporary_authorizations_cb, NULL);
}
...
static void stop_cb(void) { g_main_loop_quit(loop); }

static gboolean end_session_response(gboolean is_okay, const gchar *reason) {
  GVariant *res;
  GError *error = NULL;

  res = g_dbus_proxy_call_sync(client_proxy, "EndSessionResponse",
...
static void query_end_session_cb(void) { end_session_response(TRUE, ""); }
...
static void signal_cb(GDBusProxy *proxy, gchar *sender_name, gchar *signal_name,
...
static gboolean register_client_to_gnome_session(void) {
  GError *error = NULL;
  GVariant *res;
  ...
  sm_proxy = g_dbus_proxy_new_for_bus_sync(
      ...
      SM_DBUS_NAME, SM_DBUS_PATH, SM_DBUS_INTERFACE, NULL, /* GCancellable */
      &error);
  if (sm_proxy == NULL) {
  ...
  client_proxy = g_dbus_proxy_new_for_bus_sync(
      G_BUS_TYPE_SESSION, G_DBUS_PROXY_FLAGS_NONE,
      NULL, /* GDBusInterfaceInfo */
      SM_DBUS_NAME, client_id, SM_CLIENT_DBUS_INTERFACE,
      NULL, /* GCancellable */
      &error);
  g_variant_unref(res);
...
int main(int argc, char **argv) {
  ...
  g_signal_connect(authority, "changed", G_CALLBACK(on_authority_changed),
                   NULL);
  ...
  if (!polkit_agent_listener_register(
          listener, POLKIT_AGENT_REGISTER_FLAGS_NONE, session,
          "/org/mate/PolicyKit1/AuthenticationAgent", NULL, &error)) {
...
out:
  ...
  if (session != NULL) g_object_unref(session);
