...
G_DEFINE_TYPE(PolkitMateAuthenticator, polkit_mate_authenticator,
              G_TYPE_OBJECT);

static void polkit_mate_authenticator_init(
    PolkitMateAuthenticator *authenticator) {}

static void polkit_mate_authenticator_finalize(GObject *object) {
  PolkitMateAuthenticator *authenticator;
  ...
  if (authenticator->details != NULL) g_object_unref(authenticator->details);
  ...
  if (authenticator->session != NULL) g_object_unref(authenticator->session);
  if (authenticator->dialog != NULL) gtk_widget_destroy(authenticator->dialog);
  if (authenticator->loop != NULL) g_main_loop_unref(authenticator->loop);
...
static void polkit_mate_authenticator_class_init(
    PolkitMateAuthenticatorClass *klass) {
   ...
   **/
  signals[COMPLETED_SIGNAL] =
      g_signal_new("completed", POLKIT_MATE_TYPE_AUTHENTICATOR,
                   G_SIGNAL_RUN_LAST, 0, /* class offset     */
                   NULL,                 /* accumulator      */
                   NULL,                 /* accumulator data */
                   g_cclosure_marshal_generic, G_TYPE_NONE, 2, G_TYPE_BOOLEAN,
                   G_TYPE_BOOLEAN);
}

static PolkitActionDescription *get_desc_for_action(PolkitAuthority *authority,
  ...
  for (l = action_descs; l != NULL; l = l->next) {
    ...
    if (strcmp(polkit_action_description_get_action_id(action_desc),
               action_id) == 0) {
...
static void on_dialog_deleted(GtkWidget *widget, GdkEvent *event,
                              gpointer user_data) {
...
static void on_user_selected(GObject *object, GParamSpec *pspec,
                             gpointer user_data) {
  PolkitMateAuthenticator *authenticator = POLKIT_MATE_AUTHENTICATOR(user_data);
...
PolkitMateAuthenticator *polkit_mate_authenticator_new(
  ...
  authenticator = POLKIT_MATE_AUTHENTICATOR(
      g_object_new(POLKIT_MATE_TYPE_AUTHENTICATOR, NULL));
  ...
  if (details != NULL) authenticator->details = g_object_ref(details);
  ...
  authenticator->action_desc =
      get_desc_for_action(authenticator->authority, authenticator->action_id);
  if (authenticator->action_desc == NULL) goto error;

  authenticator->users =
      g_new0(gchar *, g_list_length(authenticator->identities) + 1);
  ...
  authenticator->dialog = polkit_mate_authentication_dialog_new(
      ...
      authenticator->icon_name, authenticator->message, authenticator->details,
      authenticator->users);
  g_signal_connect(authenticator->dialog, "delete-event",
                   G_CALLBACK(on_dialog_deleted), authenticator);
  g_signal_connect(authenticator->dialog, "notify::selected-user",
                   G_CALLBACK(on_user_selected), authenticator);
...
static void session_request(PolkitAgentSession *session, const char *request,
  ...
  // g_debug ("in conversation_pam_prompt, request='%s', echo_on=%d", request,
  // echo_on);
  ...
  if (g_ascii_strncasecmp(request, "password:", 9) == 0) {
    if (strcmp(g_get_user_name(), authenticator->selected_user) != 0) {
      modified_request =
          g_strdup_printf(_("_Password for %s:"), authenticator->selected_user);
    } else {
  ...
  if (GDK_IS_X11_WINDOW(
          gtk_widget_get_window(GTK_WIDGET(authenticator->dialog))))
  ...
  password = polkit_mate_authentication_dialog_run_until_response_for_prompt(
      POLKIT_MATE_AUTHENTICATION_DIALOG(authenticator->dialog),
      modified_request, echo_on, &authenticator->was_cancelled,
...
static void session_show_error(PolkitAgentSession *session, const gchar *msg,
                               gpointer user_data) {
  PolkitMateAuthenticator *authenticator = POLKIT_MATE_AUTHENTICATOR(user_data);
  gchar *s;
  ...
  polkit_mate_authentication_dialog_set_info_message(
      POLKIT_MATE_AUTHENTICATION_DIALOG(authenticator->dialog), s);
  g_free(s);
}

static void session_show_info(PolkitAgentSession *session, const gchar *msg,
                              gpointer user_data) {
  PolkitMateAuthenticator *authenticator = POLKIT_MATE_AUTHENTICATOR(user_data);
  gchar *s;
...
static void session_completed(PolkitAgentSession *session,
                              gboolean gained_authorization,
                              gpointer user_data) {
  PolkitMateAuthenticator *authenticator = POLKIT_MATE_AUTHENTICATOR(user_data);
...
static gboolean do_initiate(gpointer user_data) {
  ...
  gtk_widget_show_all(GTK_WIDGET(authenticator->dialog));
  gtk_window_present(GTK_WINDOW(authenticator->dialog));
  if (!polkit_mate_authentication_dialog_run_until_user_is_selected(
          POLKIT_MATE_AUTHENTICATION_DIALOG(authenticator->dialog))) {
...
try_again:
  ...
  authenticator->session =
      polkit_agent_session_new(identity, authenticator->cookie);
  ...
  if (!authenticator->gained_authorization && !authenticator->was_cancelled) {
    if (authenticator->dialog != NULL) {
      ...
      s = g_strconcat(
          "<b>",
          _("Your authentication attempt was unsuccessful. Please try again."),
          "</b>", NULL);
      polkit_mate_authentication_dialog_set_info_message(
          POLKIT_MATE_AUTHENTICATION_DIALOG(authenticator->dialog), s);
      g_free(s);
...
void polkit_mate_authenticator_initiate(
    PolkitMateAuthenticator *authenticator) {
  /* run from idle since we're going to block the main loop in the dialog (which
   * has a recursive mainloop) */
...
const gchar *polkit_mate_authenticator_get_cookie(
    PolkitMateAuthenticator *authenticator) {
